 ...
 * LastEditTime : 2023-01-06 20:17:43
 * FilePath     : \\FmtLog\\test\\test_enc_dec_test.cpp
...
void test(const S& format, Args&&... args)
{
    ...
    auto unnamed_format = UnNameFormat<false>(sv, nullptr, args...);
    // auto unnamed_format = fmtlog::unNameFormat<false>(sv, nullptr, args...);
    fmt::print("unnamed_format: {}\n", unnamed_format);
    size_t      cstringSizes[1000];
    char        buf[1024];
    int         allocSize = GetArgSizes<0>(cstringSizes, args...);
    const char* ret       = EncodeArgs<0>(cstringSizes, buf, std::forward<Args>(args)...);
    ...
    int                                         argIdx = -1;
    std::vector<fmt::basic_format_arg<Context>> format_args;
...
int main()
{
    ...
    test("test basic types: {}, {}, {}, {}, {}, {}, {}, {}, {:.1f}, {}, {}, {}, {}, {}, {}, {}", cstring, p, pcstring, "wow", 'a', 5, str, string_view(str), 1.34, ch, ch2, i, ri, d, f, short_int);

    test("test positional, {one}, {two:>5}, {three}, {four}, {0:.1f}", 5.012, "three"_a = 3, "two"_a = "two", "one"_a = string("one"), "four"_a = string("4"));
